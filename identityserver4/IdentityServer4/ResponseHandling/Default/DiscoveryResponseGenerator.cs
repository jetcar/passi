...
namespace IdentityServer4.ResponseHandling.Default
{
    ...
    public class DiscoveryResponseGenerator : IDiscoveryResponseGenerator
    {
        ...
        /// </summary>
        public virtual async Task<IEnumerable<IdentityServer4.Models.JsonWebKey>> CreateJwkDocumentAsync()
        {
            var webKeys = new List<IdentityServer4.Models.JsonWebKey>();
            ...
            foreach (var key in await Keys.GetValidationKeysAsync())
            {
                if (key.Key is X509SecurityKey x509Key)
                {
                    ...
                    if (x509Key.PublicKey is RSA rsa)
                    {
                        ...
                        var rsaJsonWebKey = new IdentityServer4.Models.JsonWebKey
                        {
                            kty = "RSA",
                            use = "sig",
                            kid = x509Key.KeyId,
                ...
                else if (key.Key is RsaSecurityKey rsaKey)
                {
                    ...
                    var webKey = new IdentityServer4.Models.JsonWebKey
                    {
                        kty = "RSA",
                        use = "sig",
                        kid = rsaKey.KeyId,
